...
return new class extends Migration {
  public function up(): void {
    Schema::create('proveedor_mensajes', function (Blueprint $t) {
      ...
      $t->enum('direccion', ['saliente','entrante']);
      $t->string('asunto');
      $t->text('cuerpo')->nullable();                 // HTML o texto plano - VJ
      $t->string('de_email',160)->nullable();
      $t->string('para_email',160)->nullable();
      $t->text('cc')->nullable();
      $t->dateTime('fecha');                          // fecha del envio/recepción - VJ
      $t->string('message_id',255)->nullable()->unique(); // para evitar duplicados al sincronizar IMAP
      $t->string('in_reply_to',255)->nullable();
      $t->string('carpeta',80)->nullable();
      $t->json('adjuntos')->nullable();               // rutas locales de adjuntos guardados - VJ
